  ...
  return (
    ...
    >
      <motion.div variants={item} className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-white">Query Audit Logs</h2>
          <p className="text-slate-400 mt-1">Complete audit trail of all Clean Room queries</p>
        </div>
      ...
      <motion.div variants={item} className="glass-panel rounded-xl overflow-hidden">
        <div className="p-4 border-b border-slate-700/50 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500">{auditLogs.length} total records</span>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700/50 bg-slate-800/30">
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Query Template</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Executor</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Result Summary</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Privacy</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">K-Anon</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Rows</th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-300 text-xs">{log.executor_org}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs ${log.privacy_checks_passed ? 'text-slate-300' : 'text-red-400'}`}>
                        {log.result_summary}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      {log.k_anonymity_met ? (
                        <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 text-xs">≥10</span>
                      ) : (
                        <span className="px-2 py-0.5 rounded bg-red-500/20 text-red-400 text-xs">N/A</span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-slate-300 font-mono text-xs">{log.rows_returned}</span>
                    </td>
                  </motion.tr>
                );
              })}
          ...
          <div className="space-y-3">
            {auditLogs.filter(l => !l.privacy_checks_passed).map((log) => (
              <div key={log.id} className="p-3 rounded-lg bg-red-500/10 border border-red-500/30">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-slate-500">{log.executor_org}</span>
                </div>
                <p className="text-sm text-white">{log.result_summary}</p>
                <p className="text-xs text-slate-400 mt-1">Reason: Attempted unapproved query template</p>
              </div>
            ))}
            {auditLogs.filter(l => !l.privacy_checks_passed).length === 0 && (
              <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-center">
                ...
                <p className="text-sm text-emerald-400">No rejected queries</p>
              </div>
            )}
          </div>
        </div>
